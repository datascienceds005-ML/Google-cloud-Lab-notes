...
def _apply_domain_fixes(text):
    """Deterministic correction for recurring mishearings that survive prompt
    bias (e.g. 'depreping genocide, arautaria'). Extend _DOMAIN_FIXES as new
    cases surface; correct spellings like 'Aratare' are never re-matched."""
...
def run_whisper_raw(audio_path, model_size="small.en", glossary="",
    ...
    duration = len(y) / sr
    # PATCH T5-PROMPT: domain hint always precedes the user glossary.
...
def _is_band_limited(y_seg, sr=16000, frame_sec=0.5, min_frac=0.50,
    ...
    """Tactical-comms detector (v3.6, frame-level). Military radio/intercom
    is hard band-limited to ~3.4 kHz: virtually no spectral energy survives
    above 5 kHz, while clean speech always carries sibilant/fricative energy
    there. ~0.5 s frames are scored individually and the segment is flagged
    when at least `min_frac` of frames are band-limited, so comms audio
    diluted by clean room audio inside one segment is still caught. Clean
    female speech has broadband frames throughout — Fran Heller is never
    flagged, so the Female stream stays intact."""
    try:
        ...
        while a + win <= x.size:
            ...
            total += 1
            a += step
        if total and hits >= min_frac * total:
            return True
        # Whole-segment roll-off: >=95% of ALL energy below ~3.8 kHz is
        # essentially impossible in clean multi-second speech.
...
def run_diarization(audio_path, raw_segments, speaker_count_override=0):
    ...
    # PATCH T5-COMMS-OVERRIDE: pitch from band-limited radio audio is skewed
    # upward, so a comms segment can NEVER carry a Female label — even when
    # clustering placed it inside a female cluster. Such segments move to a
    # dedicated Male comms bucket with its own Male_N label + stats row.
    speaker_of = {i: label_name[labels_list[i]] for i in range(len(segs))}
    comms_idxs = [i for i in range(len(segs))
                  if band_flags[i] and speaker_of[i].startswith("Female")]
    if comms_idxs:
        male_n += 1
